...
tf.app.flags.DEFINE_string(
    'train_dir', '/home/dladmin/Documents/arthurma/rf/run_hl1_tau_00001_e_1000_followirr_per/',
    'Directory where checkpoints, info, and event logs are written to.')

tf.app.flags.DEFINE_string(
    'pretrained_checkpoint_path',
    "/home/dladmin/Documents/arthurma/rf/run_hl1_tau_00001_e_1000_followirr/-4692",
    'The path to a checkpoint from which to fine-tune. Only if restore_latest_checkpoint is false')

tf.app.flags.DEFINE_boolean(
    'use_restore_dict',
    False,
    'For partial restore,update dictionary in network_actor.py')
...
tf.app.flags.DEFINE_integer(
    'pre_train_steps', 200000,  # to small will lead to divergence because of non i i d samples
...
tf.app.flags.DEFINE_float(
    'tau', 0.00001,  # 0.00001 # too large will lead to divergence because target network follows main network too fast
...
tf.app.flags.DEFINE_float(
    'start_e_greedy', 1.0,
    'epsilon greedy epxploration strategy (start probability for exploration)')
tf.app.flags.DEFINE_float(
    'end_e_greedy', 0.1,
    'epsilon greedy epxploration strategy (start probability for exploration)')
tf.app.flags.DEFINE_integer(
...
def get_train_feed_dict(network, batch, targetQ, learning_rate):
    ...
    next_train_image_sequence_batch = np.stack(np.stack(batch[:, 0])[..., 0])
    next_train_curr_irr_batch = np.stack(np.stack(batch[:, 0])[..., 1])
    next_train_control_input_batch = np.reshape(np.stack(np.stack(batch[:, 0])[..., 2]), [-1, 1])
...
def main(_):
    ...
    env = Environment(train_set_path=FLAGS.train_set_path, test_set_path=FLAGS.test_set_path,
                      episode_length=FLAGS.episode_length, sequence_length=FLAGS.img_sequence_length,
            ...
            while done == 0 and not abort:  # within each episode:
                ...
                if total_steps > FLAGS.pre_train_steps:  # create enough samples in experience replay buffer before training
                    ...
                    if total_steps % (FLAGS.update_frequency) == 0:
                        ...
                        _, gradients, time_diff_error, total_loss, q_values, lr = sess.run(
                            [mainQN.updateModel,mainQN.gradients, mainQN.td_error, mainQN.loss, mainQN.Q, mainQN.learning_rate],
                            feed_dict=feed_dict)
            ...
            if total_steps > FLAGS.pre_train_steps:
                ...
                if (episode_nr + 1) % FLAGS.validation_each_n_episodes == 0:
                    do_validation_run(train_writer=train_writer, sess=sess, network=mainQN, env=env,
                                      step=(episode_nr + 1))
        """
        print("add_priority", global_exp_buffer.priority_add_mean / global_exp_buffer.priority_add_step)
        print("sample_priority", global_exp_buffer.priority_sample_mean / global_exp_buffer.priority_sample_step)

        action_counter = collections.Counter(global_exp_buffer.chosen_samples)
        for elem, cnt in action_counter.items():
            print(elem, global_exp_buffer.get_node_priority(elem), cnt)
        """
