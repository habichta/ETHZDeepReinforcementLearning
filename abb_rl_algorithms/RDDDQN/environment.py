...
class Environment():
    def __init__(self, train_set_path, test_set_path, solar_station=ac.ABB_Solarstation.C, image_size=84,
                 ...
                 follow_irr_actions=False, file="rl_data.csv",load_train_episodes=None,load_test_episodes=None,mask_path=None,divide_image_values=None,sample_training_episodes=None):
    ...
    def reset(self):
        ...
        if (len(self.train_episode_samples) > 1):
            ...
            #Random:
            current_control_input = np.random.uniform(100.0,1000.0)
    ...
    def __calculate_next_control_input_follow_irr_straight0(self, action, current_state, next_state, current_control_input,fix_step=False):
        ...
        if action == 0:  # default action, follow current/next irradiance if possible
            ...
            diff = next_irr - current_control_input
            if abs(diff) > step:  # current control input is too far away to get to next irradiance value
                next_control_input = current_control_input # go straight if irr not reachable
            else:
                next_control_input = next_irr
    ...
    def __calculate_step_reward(self, next_irr, next_control_input):
        ...
        return -np.abs(
            next_irr - next_control_input)  # reward is negative difference. Maximizing reward is equal to reducing the difference to irr.
    ...
    def mpc_exploration(self, mpc_prob=0.5, num_actions=3):
        ...
        for a in range(num_actions):
            control_inputs.append(
                abs(self.__calculate_next_control_input_follow_irr(a, current_state, next_state, current_control_input) - mpc))
