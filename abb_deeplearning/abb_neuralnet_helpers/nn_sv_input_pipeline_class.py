...
class ABBTFInputPipeline:
    ...
    def __init__(self, tfrecord_file_path_list, image_height, image_width, image_channels, img_num_per_sample,
                 label_key_list,tfreader_options=default_options):
    ...
    """
    def get_example(self, decoded_features, feature_list, img_path_keys=None,
                    image_preprocessor=__prepr_functions_default):
        ...
        with tf.name_scope('decode_image', [image_paths], None):
            ...
            dec_image_files = [tf.cast(tf.image.decode_jpeg(file, fancy_upscaling=True, channels=self.IMAGE_CHANNELS),dtype=tf.float32) for file
                               in
                               image_files]
            #seed = tf.random_uniform(1, minval=0, maxval=None, dtype=tf.int32, seed=None, name=None) #generate seed so random preprocessing is done the same for all images in an example
            preprocessed_images = [self.__preprocess_jpeg(file, preprocessor=image_preprocessor) for file in
                                   dec_image_files]
    ...
    def get_multiple_example(self, decoded_features_list, feature_list, img_path_keys=None,
        ...
        for decoded_features in decoded_features_list:
            ...
            image_tensor = self.stack_images(image_tensor, axis=stack_axis)
    ...
    def get_preprocessed_input_shuffled_batch(self, image_preprocessor=None, num_epochs=None,
                                              ...
                                              min_batches_in_queue=4000, num_of_tfrecord_readers=100, stack_axis=2):
        ...
        tensor_list = self.get_multiple_example(decoded_features_list=decoded_fs,
                                                           feature_list=self.label_key_list,
                                                           img_path_keys=self.img_path_keys,
                                                           image_preprocessor=image_preprocessor,
                                                           stack_axis=stack_axis)
        imageBatch, labelBatch, path_list = self.create_joined_shuffled_data_batch(tensor_list, batch_size=batch_size,
                                                                        capacity=min_batches_in_queue + 10000,
                                                                        min_after_dequeue=min_batches_in_queue)

        return imageBatch, labelBatch, path_list
