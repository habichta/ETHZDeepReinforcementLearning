...
def get_vgg_model(img_rows, img_cols, img_channels):
    vgg = Sequential()
    vgg.add(ZeroPadding2D((1,1),input_shape=(img_rows, img_cols, img_channels)))
    ...
    vgg.add(Dense(4096, activation='relu'))
    vgg.add(Dropout(0.5))
    vgg.add(Dense(4, activation='softmax'))
    return vgg

def get_simple_cnn(img_rows,img_cols,img_channels,nb_classes):
    model = Sequential()
...
def get_alexnet_model(img_rows,img_cols,img_channels,nb_classes):
    model = Sequential()
    model.add(Convolution2D(48, 11, 11, input_shape=(img_rows, img_cols, img_channels), border_mode='same'))
...
#    json_file = open('/home/maverick/knet/models/params.json',"r")
    ...
    vgg = get_vgg_model(img_rows*sequence_length, img_cols, img_channels)
...
#    class weights param can help in case of un-balanced classes by balancing loss function
...
#    vgg.fit_generator(training_data_generator, 
...
#    print (vgg.summary())
    ...
    #################TEST####################
#    simple_cnn = get_simple_cnn(img_rows*sequence_length, img_cols, img_channels, nb_classes)
#    simple_cnn.compile(loss='categorical_crossentropy',
#              optimizer='rmsprop',
#              metrics=['accuracy'])
#    
#    simple_cnn.fit_generator(training_data_generator, 
#                    samples_per_epoch=samples_per_epoch, 
#                    nb_epoch = nb_epoch,
#                    verbose=1, 
#                    validation_data=validation_data_generator,
...
#    early_stopper = EarlyStopping(monitor='val_acc', min_delta=0.001, patience=15)
#    lr_reducer = ReduceLROnPlateau(monitor='val_loss', factor=np.sqrt(0.1), cooldown=0, patience=5, min_lr=0.5e-6)
#
#    alexnet = get_alexnet_model(img_rows*sequence_length, img_cols, img_channels, nb_classes)
...
#   
#    alexnet.fit_generator(training_data_generator, 
#                   samples_per_epoch=samples_per_epoch, 
#                   nb_epoch = nb_epoch,
#                   verbose=1, 
#                   validation_data=validation_data_generator,
...
#    filepath="checkpoint/weights-improvement-{epoch:02d}-{val_acc:.2f}.hdf5"
    ...
    # resnet_model.fit_generator(training_data_generator, 
    #                  samples_per_epoch=samples_per_epoch, 
    #                  nb_epoch = nb_epoch,
    #                  verbose=1, 
    #                  validation_data=validation_data_generator,
    ...
    resnet_model.load_weights("/home/pdinesh/knet-euryale/euryale/checkpoint/poop/MS_RS18_6chan_2class_pooptrained_m7-13__weights.best.hdf5")
...
#    accuracy_score(predict_labels, predict, normalize=False)
    ...
    np.save('/home/pdinesh/knet-euryale/models/checkpoint/dummy/predict_model.npy', predict)
    np.save('/home/pdinesh/knet-euryale/models/checkpoint/dummy/predict_true.npy', predict_labels)
