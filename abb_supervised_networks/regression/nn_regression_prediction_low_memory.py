...
tf.app.flags.DEFINE_string(
    'cpk_dir', '/home/dladmin/Documents/arthurma/runs/RESNET7_irr_nopool_absmax/simple_2_balFalse_typeB_regression_resnetV218_irr_ollinear11_irr_kpnodropout_32_diffFalse_ch3_isTrue_imTrue_pool_layerFalse_lfabs_max_maxw0.5_4_HARD_520410/-520410',
...
eval_file_columns = ['P0', 'P1', 'P2', 'P3', 'P4', 'P5', 'P6', 'P7', 'P8', 'P9', 'P10',
...
tf.app.flags.DEFINE_string(
    'network_architecture', 'regression_resnetV218_irr', 'The name of the architecture to train.')
...
def _configure_network(input_batch, labels, is_training):
    ...
    if FLAGS.output_layer == 'linear11_irr':
    ...
    with tf.Session(config=config) as sess:
        ...
        try:
            ...
            print("EVAL: Time: " + time_now + ", Step: " + str(step) + \
                  ...
                  ", Mean change test losses= " + str(mean_change_val_losses))

            with open(os.path.join(FLAGS.eval_dir, "eval_results.csv"), "w+") as f:
                w = csv.writer(f)
                w.writerow([str(dt.datetime.now())])
                for key, val in [("AVG MSE",avg_loss),("MED MSE",med_loss),("STD MSE",std_loss),("PERC MED",str(percentiles)),("MCTL",str(mean_change_val_losses))]:
                    w.writerow([key, val])
...
def main(_):
    ...
    full_labels = labels
    labels = tf.reshape(labels[:, 1:12], (-1, 11))
