...
tf.app.flags.DEFINE_string(
    'network_architecture', 'regression_resnetV218',
...
tf.app.flags.DEFINE_string(
    'output_layer', 'linear11', 'input layer selector') #resnet needs linear11 or linear11_irr
...
tf.app.flags.DEFINE_integer(
    'num_train_queuerunners', 3,
    'Number of queue runners that fill output train fifo queue')

tf.app.flags.DEFINE_integer(
    'capacity_batches_in_train_output_queue', 1000,
...
tf.app.flags.DEFINE_integer(
    'min_batches_in_test_queue', 50, #TODO 50000 or keep ...
    'shuffles the day tfrecords in the queue')

tf.app.flags.DEFINE_integer(
    'num_of_test_tfrecord_readers', 1, #TODO 5
    'The number of parallel readers that read data from the dataset.')
...
tf.app.flags.DEFINE_integer(
    'capacity_batches_in_test_output_queue', 100,
...
tf.app.flags.DEFINE_float('opt_epsilon', 1e-3, 'Epsilon term for the optimizer.')
...
def _create_weight_tensor(labels, change_weight, same_weight, is_training):
...
def _configure_network(input_batch, labels, is_training):
    ...
    if FLAGS.network_architecture == 'regression_resnetV218':
        ...
        network_output, end_points_network = network_factory['regression_resnetV218_bottleneck'](inputs, FLAGS.weight_decay,
                                                                                      FLAGS.batch_norm_decay,
                                                                                      FLAGS.batch_norm_epsilon,
                                                                                      FLAGS.batch_norm_scale,
                                                                                      FLAGS.global_pool,
        ...
        print("Network Architecture:",'regression_resnetV218_irr_bottleneck')
...
def main(_):
    ...
    # TODO: ugly hack, in case the validation list is empty, the string_input_producer would crash (thanks QueueRunners..). This prevents it, given
    ...
    #only if balance_weights option is set to True, otherwise both change_weight and same_weight are 1
    loss_weights = _create_weight_tensor(labels, change_weight, same_weight, is_training) # default, creates loss weights of 1 if balancing is switched off
