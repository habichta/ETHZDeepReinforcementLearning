...
class RealCloudEnvironmentNoEps():
    def __init__(self, data_path,img_path,train_set_path, image_size=84,
                 sequence_length=4, sequence_stride=9, action_nr=7, action_type=1,adapt_step_size=True, ramp_step=0.1, episode_length_train=None,
                  file="rl_data_sp.csv",load_train_episodes=None,mask_path=None,exploration_follow="IRR",start_exploration_deviation=0.2,clip_irradiance=False,filter_eps=True):
        ...
        # Episodes:
        self.train_start_sample_df,self.train_data_df= self.__create_episodes(train_set_path=train_set_path)
    ...
    def reset(self):
        ...
        self.current_episode_train = self.__sample_episode()

        assert len(set(self.current_episode_train.index.date)) == 1
    ...
    def __sample_episode(self):
        ...
        while not legal_sample:
            start_ind = np.random.randint(0,len(self.train_start_sample_df.index), size=1)[0]
            start = self.train_start_sample_df.index[start_ind]
            start_int = self.train_data_df.index.get_loc(start)
            end_int = start_int+self.episode_length_train
            episode = self.train_data_df.iloc[start_int:end_int]
            done = pd.DataFrame(0, index=episode.index, columns=['done'])
            episode = pd.concat([episode,done],axis=1)
            episode.iloc[-1, episode.columns.get_loc('done')] = 1

            legal_sample = self.filter_episodes(episode)
    ...
    def __create_episodes(self, train_set_path):
        ...
        if self.clip_irradiance:  # changes irradiance to 1.0 or 0.0. 1.0 if irradiance is larger than 70% of clear sky model
            rl_pd['irr'] = np.where(rl_pd['cs'] * 0.7 <rl_pd['irr'], 1.0, 0.0)
        ...
        cut_off_end = self.episode_length_train-1
        train_data = rl_pd[rl_pd.index.normalize().isin(self.train_list)].sort_index()

        train_set = train_data.drop(train_data.groupby(train_data.index.date).tail(cut_off_end).index)
    ...
    def filter_episodes(self, df):
        ...
        if self.filter_eps:
            t = pd.DataFrame()
            t['tvalue']  = df.index
            t['delta'] = (t['tvalue'] - t['tvalue'].shift()).fillna(0)
            if np.max(t['delta']/ np.timedelta64(1,'s')) > 14.0: # check there are not too large  time differences between samples
                keep = False
    ...
    class ActionSpace():
        ...
        def calculate_step(self, action, next_irr, curr_irr, current_ci,curr_index,next_index):
            ...
            if self.type == 0:
                ...
                if action == 0:  # upper target
                    next_ci = np.minimum(1.0,float(current_ci) + ramp_step_s*stretch_factor)
                elif action == 1:  # lower target
                    next_ci = np.maximum(0.0, current_ci - ramp_step_s*stretch_factor)
                else:
                    raise ValueError('Illegal action')
